...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "Xush kelibsiz!\n📝 Post yaratish uchun tugmani bosing\n🎭 Stiker ID olish: /getid",
        reply_markup=main_kb(),
    )

async def getid(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("🎭 Endi menga stiker yuboring — file_id beraman!")

# ── Stiker handler ──────────────────────────
async def stiker_olish(update: Update, context: ContextTypes.DEFAULT_TYPE):
...
async def webapp_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if havola and tugma_nomi:
        rm = json.dumps({"inline_keyboard":[[{"text":f"{emoji} {tugma_nomi}","url":havola,"style":style}]]})

    try:
        res = None
        if media_type == "stiker" and stiker_id:
            await api_post("sendSticker", chat_id=CHANNEL_ID, sticker=stiker_id)
            if matn != "Yangi post!" or rm:
        ...
        if res and res.get("ok"):
            await update.message.reply_text(f"✅ Kanalga yuborildi! {emoji}", reply_markup=main_kb())
        else:
...
def main():
    ...
    # Stiker handler — alohida, aniq filter
    app.add_handler(MessageHandler(filters.Sticker.ALL, stiker_olish))
    # WebApp handler
