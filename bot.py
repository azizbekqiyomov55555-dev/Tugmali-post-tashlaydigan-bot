...
# ─── /getid — stiker file_id olish ───────────
async def getid(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("🎭 Menga stiker yuboring — file_id ni beraman!")

async def stiker_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    stiker = update.message.sticker
    file_id = stiker.file_id
    await update.message.reply_text(
        f"✅ Stiker file_id:\n\n<code>{file_id}</code>\n\nBu kodni webapp ga kiriting!",
        parse_mode="HTML"
    )
...
def main():
    ...
    app.add_handler(MessageHandler(filters.Sticker.ALL, stiker_handler))
