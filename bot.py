...
async def getid(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "🎭 Endi menga istalgan stiker yuboring!\n"
        "Men uning file_id sini beraman."
    )

# ─── BARCHA XABARLAR HANDLERI ─────────────────
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if msg.sticker:
        ...
        await msg.reply_text(
            f"✅ Stiker file_id:\n\n"
            f"<code>{file_id}</code>\n\n"
            f"Yuqoridagi kodni bosib nusxa oling, webapp ga kiriting!",
            parse_mode="HTML"
        )
        return
...
async def webapp_data(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if media_type == "stiker" and stiker_id:
            await send_with_style("sendSticker", CHANNEL_ID, None, sticker=stiker_id)
            if matn != "Yangi post!" or reply_markup:
                result = await send_with_style("sendMessage", CHANNEL_ID, reply_markup, text=matn)
            else:
                result = {"ok": True}
            ...
            result = await send_file_with_style(
                "sendPhoto", CHANNEL_ID, "photo", file_bytes, f"photo.{ext}",
                reply_markup, caption=matn if matn != "Yangi post!" else ""
            )
            ...
            file_bytes = base64.b64decode(encoded)
            result = await send_file_with_style(
                "sendVideo", CHANNEL_ID, "video", file_bytes, "video.mp4",
                reply_markup, caption=matn if matn != "Yangi post!" else ""
            )
        ...
        await update.message.reply_text(f"❌ Xato: {str(e)}", reply_markup=main_kb())
